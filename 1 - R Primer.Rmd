...
To display angles in degrees

3.	How is the atan2 function different from the atan function? 
atan gives angle value between -90 and 90, atan gives value between -180 and 180.

4.	What is the difference between log and log10?
log is natural log with base e, log10 is log of ten as base.
...
Sum is total of elements in a vector, and cumsum sums all the previous elements together
...
```{r}
dat = read.table("frogs.txt",header=TRUE)
```
...
```{r}
write.table(dat,"my_frogs2.csv",row.names=FALSE,sep="|")
dat2 = read.table("my_frogs2.csv",header=TRUE,sep="|")
```
...
```{r}
  superheros <- c("IronMan","SpiderMan","BatMan","AquaMan")
```
...
```{r}
apply(dat[,1:2],2,sd)  # 1.274559 2.370449 
```
...
```{r}
dat2 = dat[dat$color == "red",]
  reg1 = lm(dat2$tadpoles ~ dat2$frogs, data=dat)
  reg1
  summary(reg1)
```
...
```{r}
regc1 = lm(dat$tadpoles ~ dat$frogs, data=dat)
summary(regc1)
regc2 = lm(dat$tadpoles ~ dat$spots, data=dat)
summary(regc2) # not significant
regc3 = lm(dat$tadpoles ~ dat$color, data=dat)
summary(regc3)
regc4 = lm(dat$tadpoles ~ dat$color + dat$frogs + dat$color*dat$frogs, data=dat)
summary(regc4) # color and interaction term not significant
regc5 = lm(dat$tadpoles ~ dat$color + dat$frogs, data=dat)
summary(regc5) # color term not significant
```
...
```{r}
my.plot = function(frog,tadpole,color,spot){
plot(frog,tadpole,
     cex=1.5,  			# increase the symbol size
     col=as.character(color),	# change the symbol color by name
     pch=spot+1,			# change the symbol (by number)
     cex.axis=1.3,			# increase the font size on the axis 
     xlab="Frog Density",		# label the x axis
     ylab="Tadpole Density",		# label the y axis
     cex.lab=1.3,			# increase the axis label font size
...
abline(lm(tadpole ~ frog), col="green",		# add the regression line
     ,lwd=3)				# increase the line width
legend("topleft",
       c("Red no spot","Blue no spot","Red spots","Blue Spots"),
       pch=c(1,1,2,2),
...
dat4 = dat[dat$tadpoles >= 5,]
my.plot(dat4$frogs, dat4$tadpoles, dat4$color, dat4$spots)
```
...
N = rep(N0,NT)
for(t in 2:NT){
	N[t] = N[t-1]*r   ## discrete logistic growth
}
plot(N)
```
