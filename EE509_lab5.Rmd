...
model{
  ...
  for(i in 1:n){
	  ...
	  # not put y in, jags thinks simulate y; what if put y in? '19
...
```{r}
inits = list(S=1/var(y)) #cannot use to specify priors 
```
...
* Make a scatter plot of the data and add your posterior mean regression line
...
```{r}
plot(jags.out)
# The model converges well because as iteration increases, we see green overlaps two other chains by a large part, which means three chains overlap and converge. There are almost no burn-in and converge quickly.
gelman.diag(jags.out)
# It is less than 1.05, so converges good

GBR <- gelman.plot(jags.out) # determine burnin
...
```{r}
plot(x1,y)
lines(x1, 9.75269 + 2.01480*x1)
```

```{r}
pairs(out)	## pairs plot to evaluate parameter correlation
cor(out)    ## correlation matrix among model parameters
# From the plot, b[1] and b[2] have a high negative correlation(covariance). No correlation between S and b[1], S and b[2] 
```
...
# intercept: similar means and uncertainty
# Bayesian 9.76 vs classcial 9.77
# std: Bayesian 0.900 vs classical 0.887

# slope: 
# Bayesian 2.014 vs classcial 2.01
# std: Bayesian 0.076 vs classical 0.075

# residual se:
# Bayesian 4.09 vs classcial 4.61, classical has less uncertainty on residual standard error.
```
...
model{
  ...
  for(i in 1:n){
	  mu[i] <- b[1] + b[2]*x1[i] + b[3]*x2[i] + b[4]*x1[i]*x2[i]   	## process model
	  y[i]  ~ dnorm(mu[i],S)		        ## data model
  }
}
"
...
data2 <- list(x1 = x1, x2 = x2, y = y, n = length(y))

## specify priors
data2$b0 <- as.vector(c(0,0,0,0))      ## regression b means
# make a diagonal matrix of size 4*4, SD = 100, precision = 1/10000
data2$Vb <- solve(diag(10000,4))   ## regression b precisions
data2$s1 <- 0.1                    ## error prior n/2
data2$s2 <- 0.1                    ## error prior SS/2

inits2 <- list()
for(i in 1:nchain){
 inits2[[i]] <- list(b = rnorm(4,0,5), S = runif(1,1/200,1/20))
}
```
...
burnin2 = 2000                                ## determine convergence
...
pairs(out2)	## pairs plot to evaluate parameter correlation
cor(out2)    ## correlation matrix among model parameters
```