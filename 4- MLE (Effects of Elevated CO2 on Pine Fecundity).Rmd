...
```{r}
b <- read.table("/Users/xukaiyan/Desktop/EE509/pinecone.txt",header=TRUE)
...
```{r,echo=FALSE}
    ...
    param = c(b0,b1)
    plot(dia,cones)
    lines(xseq,pnorm(xseq,param[1],param[2]))
```
...
```{r}
likFec2 = function(param,dia,cones){
  theta <- pnorm(dia,param[1],param[2])
  -sum(dbinom(cones, 1, theta, log=TRUE)) ## negative log likelihood
}

mature = likFec2(param,dia,cones)

opt = optim(c(median(dia),sd(dia)),likFec2,dia=dia,cones=cones)
opt
```
...
```{r}
xseq2 = seq(min(dia),max(dia),length = 300)
g = out$par[1]*xseq2^2 
theta = pnorm(xseq2,out$par[2],out$par[3])
plot(dia,ncone)
lines(xseq2,g,col='2', )
lines(xseq2,g*theta,col='3')
legend(1, 95, legend=c("size", "cones"),
       col=c("2", "3"), lty=1)

plot(dia,cones)
lines(xseq2,theta,col='4')
```
...
```{r}
dia.elev   = b$diam[tmt=="CO2"] 
ncone.elev = b$c00[tmt=="CO2"] 
...
n[3]   = length(ncone.elev)
out.elev
```
```{r}
tab <- matrix(c(a0[1], a0[2], a0[3], b0[1], b0[2], b0[3], b1[1], b1[2], b1[3], n[1],n[2],n[3],lnL[1],lnL[2],lnL[3]), ncol=3, byrow=TRUE)
colnames(tab) <- c('All data','Ambient','Elevated')
rownames(tab) <- c('a0','b0','b1', 'n', 'lnL')
tab <- as.table(tab)
tab
```
...
```{r}
plot(dia,ncone)
lines(xseq2,a0[1]*xseq2^2,col='2')
lines(xseq2,a0[2]*xseq2^2,col='3')
lines(xseq2,a0[3]*xseq2^2,col='4')
```

```{r}
plot(dia,cones)
lines(xseq2,pnorm(xseq2,b0[1],b1[1]),col='2')
lines(xseq2,pnorm(xseq2,b0[2],b1[2]),col='3')
lines(xseq2,pnorm(xseq2,b0[3],b1[3]),col='4')
```

```{r}
plot(dia,ncone)
lines(xseq2,(a0[1]*xseq2^2)*pnorm(xseq2,b0[1],b1[1]),col='2')
lines(xseq2,(a0[2]*xseq2^2)*pnorm(xseq2,b0[2],b1[2]),col='3')
lines(xseq2,(a0[3]*xseq2^2)*pnorm(xseq2,b0[3],b1[3]),col='5')
```


1. Does elevated CO2 have an effect on fecundity? (Provide explanation for your answer).
Yes. Based on the fecundity graph, a0 in elevated areas is higer across all the diameters, so fecundity is higher in elevated areas.
...
For the maturation graph, cones in elevated areas have earlier maturation rate than cones in ambient areas do. Therefore, elevated CO2 has positive effects on both fecundity and maturation.
