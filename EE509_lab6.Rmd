...
```{r}
load("/Users/xukaiyan/Desktop/Lab6.RData")
library(mvtnorm)
```
...
beta=c(5,100)
plot(L, grow)
X <- seq(min(L), max(L), length=130)
lines(X,beta[1] + beta[2]*X/(theta + X))
```
...
```{r}
out  = as.mcmc(cbind(bgibbs,sgibbs,tgibbs))

par(mar=c(5.1/2, 4.1/2, 4.1/2, 2.1/2))
plot(out)
# I decide to burn first 500 because in some plots parameters (b[0],b[1],s) begin to converge after 500 samples

burn <- window(out,start=500)  ## remove burn-in
plot(burn)
summary(burn)

par(mar=c(5.1, 4.1, 4.1, 2.1))
acfplot(burn, lag.max = 500)
# I decide to thin every 150 samples because autocorrelation is 0 in some plots.

thin = window(burn,thin=150)
par(mar=c(5.1/2, 4.1/2, 4.1/2, 2.1/2))
plot(thin)

effectiveSize(burn)

cumuplot(burn,probs=c(0.025,0.25,0.5,0.75,0.975))

out2 <- as.matrix(burn)
pairs(out2)
cor(out2)
# nonlinear relationship between tgibbs and b[0]
```
...
```{r}
accrate <- 1-rejectionRate(out)
accrate
# At jumpSD = 0.1, Acceptance rate = 17.1%
# At jumpSD = 0.05, Acceptance rate = 28.2%
# At jumpSD = 0.025, Acceptance rate = 42.8%
# At jumpSD = 0.015, Acceptance rate = 59.3%
# I decide to choose 0.025 because it is between 30% - 70%, so it is neither not mixing or taking too much time to mix.
```
...
model{
  ...
  b ~ dmnorm(b0,vinvert)  	## multivariate Normal prior on vector of regression params
  S ~ dgamma(s1,s2)    ## prior precision
  t ~ dbeta(a1, a2)

  for(i in 1:n){
	  mu[i] <- b[1] + b[2]*x[i]/(t+x[i])   	## process model
	  grow[i] ~ dnorm(mu[i],S)		        ## data model
  }
}
"

data <- list(x = L, grow = grow, n = length(grow))
data$b0 <- as.vector(c(0,0))
data$vinvert <- solve(diag(1000,2))
data$s1 <- 0.1
data$s2 <- 0.1
data$a1 = 1.91
data$a2 = 10.17
...
inits <- list()
for(i in 1:nchain){
inits[[i]] <- list(b = rnorm(2,0,5), S = runif(1,1/200,1/20), t = rbeta(1, 0.1, 0.1))
}
...
# also decide to burn the first 500 samples because for some plots it takes about 500 samples to go down 1.05

burnin = 500                                ## determine convergence
jags.burn <- window(jags.out,start=1500)  ## remove burn-in
par(mar=c(5.1/2, 4.1/2, 4.1/2, 2.1/2))
plot(jags.burn)
summary(jags.burn)
```
...
```{r}
par(mar=c(5.1, 4.1, 4.1, 2.1))
acfplot(jags.burn, lag.max = 500)
# I decide to thin every 100 samples because autocorrelation is 0 in some plots.

jags.thin = window(jags.burn,thin=100)
par(mar=c(5.1/2, 4.1/2, 4.1/2, 2.1/2))
plot(jags.thin)
...
jags.out2 <- as.matrix(jags.burn)
pairs(jags.out2)
cor(jags.out2)
# nonlinear relationship between b[1] and t
```
...
answer: S is substantially different. Other statistics are close in magnitude.